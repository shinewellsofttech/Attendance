...
7. [State Master](#7-state-master)
8. [City Master](#8-city-master)
9. [Global Options](#9-global-options)
...
| `F_StateMaster` | number | No | State ID (foreign key to StateMaster) |
| `F_CityMaster` | number | No | City ID (foreign key to CityMaster) |
| `PFNo` | string | No | PF (Provident Fund) number |
| `ESINo` | string | No | ESI (Employee State Insurance) number |
| `NatureOfBusiness` | string | No | Nature of business |
| `Signature` | string | No | Signature |
...
- `F_StateMaster` and `F_CityMaster` are foreign key parameters (uses F_ prefix for dropdown/foreign key)
- City dropdown is populated based on selected State
- If State is not selected, City dropdown is disabled
...
### API Endpoints
- **Save/Update**: `StateMaster/0/token`
- **Get by ID**: `{MASTER_URL}/0/token/StateMaster/Id`
- **Get List**: `{MASTER_URL}/0/token/StateMaster/Id/0`
...
- UserId comes from `localStorage.getItem("user")` → `obj.uid`
- Id is auto-generated by the backend, not required in form submission
...
### API Endpoints
- **Save/Update**: `CityMaster/0/token`
- **Get by ID**: `{MASTER_URL}/0/token/CityMaster/Id`
- **Get List**: `{MASTER_URL}/0/token/CityMaster/Id/0`

### Form Parameters (FormData)
| Parameter Name | Type | Required | Description |
|---------------|------|----------|-------------|
| `Name` | string | Yes | City name |
| `F_StateMaster` | number | Yes | State ID (foreign key to StateMaster) |
| `UserId` | number | Yes | Current user ID (from localStorage.user.uid) |
| `id` | number | Yes | Record ID (0 for new, >0 for update) |

### Notes
- All fields are required
- `F_StateMaster` is the foreign key parameter (uses F_ prefix for dropdown/foreign key)
- UserId comes from `localStorage.getItem("user")` → `obj.uid`
- Id is auto-generated by the backend, not required in form submission
...
- `F_StateMaster` - Foreign key to StateMaster
- `F_CityMaster` - Foreign key to CityMaster
